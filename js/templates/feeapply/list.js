	...
	var app = angular.module('mdfeeapplylist', [])
		.filter('order_status', function($sce) {
			...
			$scope.query = {
				...
				apply_name: null,//声请人
				begin_date: new Date(new Date().getFullYear() + '-' + new Date().getMonth() + '-' + new Date().getDate()),//开始时间
				end_date: new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate()),//结束时间
				order_status: null,//流程状态下拉框选择的值
				fee_apply_code: null//单号
			};
			$scope.loadData = function() {//查询函数
				var param = {
					"tenant_id": null,
					"user_id": null,
			...
			$scope.onDelete = function(ev) {//table外
			...
			$scope.onDeleteOne = function(ev,id) {//table内
				console.log(id);
				var confirm = $mdDialog.confirm().title('删除确认').textContent('确定删除此条数据？').ariaLabel('删除').targetEvent(ev).ok('删除').cancel('取消');
				$mdDialog.show(confirm).then(function() {
					var ids = [id];
