...
define(['angular'], function(angular) {
	var app = angular.module('md.feeapplyv2', []);
	//组织架构
	app.directive('drfeeapplyv2', ['$mdMedia', '$mdDialog', '$http', '$parse', 'publicmodel', '$filter', function($mdMedia, $mdDialog, $http, $parse, publicmodel, $filter) {
		return {
			link: function(scope, element, attrs) {
				function bindData(event) {
					var modeldata = scope.$eval(attrs.drfeeapplyv2);
					console.log(modeldata.model);
					var parsevalue = $parse(modeldata.model)(scope);
					$mdDialog.show({
						controller: DialogController,
						templateUrl: 'templates/shared/feeapplyv2.html',
					...
					function DialogController($scope, $mdDialog,$timeout) {
						...
						$scope.filter = {
							source : [
								{id:'apply_name',name:'申请人'},
								{id:'fee_apply_code',name:'单号'},
								{id:'reason_desc',name:'申请详情'}
							],
							value : {},
						...
						$scope.getData = function() {
							...
							$http({
								method: 'POST',
								noLoader:true,
								url: feelapplyUrl + 'ea/feeApply/getAvaliableFeeApply',
								data: param,
								headers: {
