define(['angular', 'js/shared/organization'], function(angular, org) {
	var app = angular.module('mdproceslist', ['mdorganization']);
	app.directive('ngProcessnode', ['$http', 'publicmodel', '$mdDialog', function($http, publicmodel, $mdDialog) {
		return {
			restrict: 'EA',
			replace: true,
			transclude: true,
			priority:9,
			templateUrl: 'templates/shared/process.html',
			'scope': {
			...
			link: function(scope, element, attrs) {
				...
				scope.opertionlist = function() {
					...
					if(forminstanceid && modelid && formtemplateid) {
						$http({
							method: 'POST',
							url: feelapplyUrl + '/wf/process/flowt',
							headers: {
