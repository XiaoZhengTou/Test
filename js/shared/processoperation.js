define(['angular', 'js/shared/organization', 'js/templates/feeapply/filter'], function(angular, org) {
	var app = angular.module('mdprocessoperation', ['mdorganization', 'mdfeeapplyfilter']);
	app.directive('ngProcessoperation', ['$http', '$mdDialog', '$parse', '$mdBottomSheet', function($http, $mdDialog, $parse, $mdBottomSheet) {
		return {
			restrict: 'EA',
			replace: true,
			//priority: 9999,
			templateUrl: 'templates/shared/processoperation.html',
			'scope': {
				'processlsit': '=',
				'forminstanceid': '=',
				'formtemplateid': '=',
			...
			link: function(scope, element, attrs) {
				...
				scope.opertionlist = function() {
					if(forminstanceid && modelid && formtemplateid) {
						...
						$http({
							method: 'POST',
							url: feelapplyUrl + 'wf/process/oplist',
							data: {
						...
						$http({
							method: 'POST',
							url: feelapplyUrl + '/wf/process/opparam',
							data: {
								'formInstanceId': forminstanceid,
								'model_id': modelid,
								'template_form_id': formtemplateid,
				...
				scope.onOpenApprove = function(ev) {
						$mdBottomSheet.show({
							templateUrl: 'templates/shared/processdialog/approve.html',
							controller: function(scope) {
								/*审批意见*/
				...
				scope.onOpenCommiss = function(ev) {
					$mdBottomSheet.show({
						templateUrl: 'templates/shared/processdialog/commiss.html',
						controller: function(scope) {
							scope.approveauditNote = null;
							scope.users = [];
				...
				scope.onOpenCommunicate = function(ev) {
					$mdBottomSheet.show({
						templateUrl: 'templates/shared/processdialog/communicate.html',
						controller: function(scope) {
							scope.approveauditNote = null;
							scope.users = [];
				...
				scope.onCelcommunicate = function() {
					...
					$http({
						method: 'POST',
						url: feelapplyUrl + 'wf/process/celcommunicate',
						data: {
							'auditNote': '取消沟通',
							'formInstanceId': forminstanceid,
							'model_id': modelid,
				...
				scope.onRecommunicate = function() {
					...
					$http({
						method: 'POST',
						url: feelapplyUrl + 'wf/process/recommunicate',
						data: {
							'auditNote': '取消沟通',
							'formInstanceId': forminstanceid,
							'model_id': modelid,
				...
				scope.onShowPressProcess = function() {
						$mdBottomSheet.show({
							templateUrl: 'templates/shared/processdialog/press.html',
							controller: function(scope) {
								scope.approveauditNote = null;
								scope.onPressProcess = function() {
									...
									$http({
										method: 'POST',
										url: feelapplyUrl + 'wf/process/press',
										data: {
				...
				scope.onShowDrafterProcess = function() {
						$mdBottomSheet.show({
							templateUrl: 'templates/shared/processdialog/drafterprocess.html',
							controller: function(scope) {
								scope.approveauditNote = null;
								scope.onDrafterProcess = function() {
									...
									$http({
										method: 'POST',
										url: feelapplyUrl + 'wf/process/drafter',
										data: {
											"auditNote": approveauditNote,
											"formInstanceId": forminstanceid,
											"template_form_id": formtemplateid,
				...
				scope.onShowDrafterProcess = function() {
					$mdBottomSheet.show({
						templateUrl: 'templates/shared/processdialog/abandon.html',
						controller: function(scope) {
							scope.approveauditNote = null;
							scope.onAbandonProcess = function() {
								...
								$http({
									method: 'POST',
									url: feelapplyUrl + 'ea/feeApply/disableFeeApply',
									data: {
										"tenant_id": "12",
										"user_id": "12",
