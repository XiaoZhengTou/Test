...
define(['angular'], function (angular) {
    ...
    var test = angular.module('company', []);
    test.controller('companyCtrl',function($scope, $log, $mdDialog, $mdMedia,NgTableParams,$http,$cookies,$filter,$mdToast,$timeout){
        ...
        function getapp1(page,size){
            var getconfig = {
                method: 'POST',
                //url: APP_CONFIG.JIEBAO_URL+'/docker/company/queryAll',
        ...
        $scope.add = function(ev) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'templates/basicdata/company-dialog1.html',
            ...
            function DialogController1($scope, $mdDialog) {
                ...
                $scope.answer = function(answer) {
                    for(var i=0;i<$scope.currencydata_form.length;i++){
                        if($scope.company.currency_code == $scope.currencydata_form[i].currency_code){
                            $scope.company.currency_name=$scope.currencydata_form[i].currency_name
                        }
                    }
                    for(var i=0;i<$scope.ou_form.length;i++){
                        if($scope.company.external_org_id == $scope.ou_form[i].id){
                            $scope.company.external_org_code=$scope.ou_form[i].company_code;
                            $scope.company.external_org_name=$scope.ou_form[i].company_name;
                            $scope.company.proxy_user=$scope.ou_form[i].proxy_user;
                        }
                    }

                    var getconfig = {
                        method: 'POST',
                        url: APP_CONFIG.JIEBAO_URL+'/docker/company',
                        headers: {
                            'x-auth-token': $cookies.get("JSESSION")
                        },
                        data:$scope.company
                ...
                function getou(){
                    var getconfig = {
                        method: 'GET',
                        url: APP_CONFIG.JIEBAO_URL+'/docker/operatingunits/queryAll',
                        headers: {
                            'x-auth-token': $cookies.get("JSESSION")
                        }
                    };
        ...
        $scope.edit = function(ev,item) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'templates/basicdata/company-dialog2.html',
                parent: angular.element(document.body),
                targetEvent: ev,
                clickOutsideToClose:true,
                fullscreen: useFullScreen
            ...
            function DialogController2($scope, $mdDialog) {
                ...
                $scope.delete = function(){
                    var getconfig = {
                        method: 'DELETE',
                        url: APP_CONFIG.JIEBAO_URL+'/docker/company/'+$scope.company.company_id,
                        headers: {
                            'x-auth-token': $cookies.get("JSESSION")
                        }
                    };
                ...
                $scope.delete1 = function(){
                    var getconfig = {
                        method: 'DELETE',
                        url: APP_CONFIG.JIEBAO_URL+'/docker/company/deletePhysical/'+$scope.company.company_id,
                        headers: {
                            'x-auth-token': $cookies.get("JSESSION")
                        }
                    };
                ...
                //var  str=item.load_date.toString();
                //str =  str.replace(/-/g,"/");
                ...
                $scope.company = {
                    ...
                    remark:item.remark,
                    manage_org_id:item.manage_org_id,
                    manage_org_code:item.manage_org_code,
                    manage_org_name:item.manage_org_name,
                    manage_org_person:item.manage_org_person,
                    external_org_id:item.external_org_id,
                    external_org_code:item.external_org_code,
