 ...
 */
define(['angular','js/view/basicdata/budgetadjust/editmoneytable.js','js/view/basicdata/budgetadjust/editmoneyprocess.js'], function (angular) {
    publicFunction.addStyle('md-data-table.min.css');
    var test = angular.module('editmoneylist', ['editmoneytable','editmoneyprocess']);
    test.config(function ($routeProvider, $locationProvider,$httpProvider) {
        $httpProvider.defaults.timeout = 5000;
        $routeProvider
            .when('/editbudget_money_table', {
                templateUrl:  'view/basicdata/budgetadjust/editmoneytable.html'
            })
            .when('/editbudget_money_process', {
                templateUrl: 'view/basicdata/budgetadjust/editmoneyprocess.html'
            })
            .otherwise({
                redirectTo: '/editbudget_money_list'
            });
    })
    test.controller('editmoneylistCtrl',function($scope, $log, $mdDialog, $mdMedia,$http,$cookies,$filter,$mdToast,$location,$timeout,$mdEditDialog){
        ...
        //搜索
        $scope.search_editmoneylist = function(){
        ...
        function getapp1(){
            var getconfig = {
                method: 'POST',
                url:  APP_CONFIG.SMART_URL + '/bm/EmsBmAdjustApplyH/listFeeAmountAdjustApply',
                headers: {
        ...
        //新增
        $scope.add = function() {
            $location.path("/editbudget_money_table").search('status','add')
        };

        //编辑
        $scope.edit = function(item) {
            $location.path("/editbudget_money_table").search('status','edit').search('adjustid',item.adjust_apply_h_id)
        };
        ...
        $scope.show = function(ev,item) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'templates/basicdata/budgetadjust/editmoneylist-dialog1.html',
    ...
    angular._LoadModule('editmoneylist');
    return test;
});