...
define(['angular','js/shared/ruzhangdanwei.js'], function (angular) {
    ...
    var test = angular.module('processauthmyewu', ['md.ruzhangdanwei']);
    test.controller('processauthmyewuCtrl',function($scope, $log, $mdDialog, $mdMedia,NgTableParams,$http,$cookies,$filter,$element,$timeout, $q,$mdToast){
        ...
        //        }
        //    }else{
        //        window.location.href=APP_CONFIG.FRONT_URL+"/index.html#/processauthadmin"
        //    }
        //});
        ...
        $scope.addperson1 = function(ev) {
            $mdDialog.show({
                ...
                if (answer) {
                    if (answer) {
                        $scope.search.from_who_name = answer.user_full_name;
                        $scope.search.from_who = answer.user_id
                    }
                }
            }, function() {
    ...
    angular._LoadModule('processauthmyewu');
    return test;
});