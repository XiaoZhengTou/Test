  ...
  function preset(size, options) {
    ...
    return {
      ...
      scope: {
        ...
        size: size,
        type:options.type,
        min:options.mdMinDate
      },
      template:
        '<md-edit-dialog>' +
          '<div layout="column" class="md-content" style="max-height: 200px;overflow: auto;">' +
            '<div ng-if="size === \'large\' && !(type === \'table11\')" class="md-title">{{title || \'Edit\'}}</div>' +
            '<form name="editDialog" layout="column" ng-submit="submit(model)">' +

        '<table class="table ng-scope" ng-if="type === \'table11\'">'+
        '<thead> <tr>'+
        '<th style="word-break: keep-all;white-space: nowrap;" class="ng-binding ng-scope">序号</th>'+
        '<th style="word-break: keep-all;white-space: nowrap;" class="ng-binding ng-scope">预估值</th>'+
        '<th style="word-break: keep-all;white-space: nowrap;" class="ng-binding ng-scope">控制进度值</th>'+
        '<th style="word-break: keep-all;white-space: nowrap;" class="ng-binding ng-scope">开始日期</th>'+
        '<th style="word-break: keep-all;white-space: nowrap;" class="ng-binding ng-scope">结束日期</th></tr></thead><tbody class="md-caption">'+
        '<tr ng-repeat="hinvit in model" class="ng-scope">'+
        '<td class="ng-binding ng-scope">{{$index+1}}</td>'+
        '<td class="ng-binding ng-scope">{{hinvit.evaluate_value}}</td>'+
        '<td class="ng-binding ng-scope">{{hinvit.process_control_value}}</td>'+
        '<td class="ng-binding ng-scope">{{hinvit.begin_date}}</td>'+
        '<td class="ng-binding ng-scope">{{hinvit.end_date}}</td></tr></tbody></table>'+

        '<div ng-if="!(type === \'table11\')">' +
              '<md-datepicker name="input" md-min-date="min" ng-model="model" md-placeholder="" style="background-color: #f9f9f9;" ng-if="type === \'date\'"></md-datepicker>' +
              '<md-input-container md-no-float ng-if="!(type === \'date\')">' +
                '<input name="input" ng-if="!(type === \'range\' || type === \'date\')" ng-model="model" md-autofocus placeholder="{{placeholder}} "' + getAttrs() + '>' +
                '<md-slider name="input" flex md-discrete ng-model="model" style="min-width: 130px" step="1" min="1" max="100" aria-label="rating" ng-if="type === \'range\'">' +
        ...
        '</form>' +
          '</div>' +
          '<div ng-if="size === \'large\' && !(type === \'table11\')" layout="row" layout-align="end" class="md-actions">' +
            '<md-button class="md-primary" ng-click="dismiss()">{{cancel}}</md-button>' +
            '<md-button class="md-primary" ng-click="submit()">{{ok}}</md-button>' +
          '</div>' +
          '<div ng-if="(type === \'table11\')" layout="row" layout-align="end" class="md-actions">' +
            '<md-button class="md-primary" ng-click="dismiss()">{{cancel}}</md-button>' +
          '</div>' +
        '</md-edit-dialog>'
    };
